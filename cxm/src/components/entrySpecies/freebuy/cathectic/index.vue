...
<template>
    <div class="wrap cathectic">
                ...
                <div class="cont">
                    ...
                    <div class="mat_cen" :id='item.matchId' :class="$route.query.playType=='3'||$route.query.playType=='5'||$route.query.playType=='7'?'lang':'short'">
                        ...
                        <ul class="spf_list" v-if="$route.query.playType=='1'||$route.query.playType=='2'">
                            ...
                            <li :class="item.myspf.indexOf(1)!=-1?'cont_cur':''"><p @click="selectedClick($event)"></p><span>vs <i :style="{'color':item.matchPlays[0].fixedOdds.substr(0,1)=='+'?'#d12120':'#44ae35'}">{{item.matchPlays[0].fixedOdds}}</i></span><span>平{{item.matchPlays[0].flatCell.cellOdds}}</span></li>
        ...
        <section class="fixed fixBottom">
            ...
            <div>
                <div class="left">
                    <p>{{betObj.betNum}}注{{betObj.times}}倍&nbsp;&nbsp;共需：<span>{{betObj.money}}</span></p><p>模拟预测奖金：<span class="ycjj">{{betObj.minBonus}}-{{betObj.maxBonus}}</span></p>
                </div>
