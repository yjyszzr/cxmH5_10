    ...
    <div class="wrap mix">
        <section>
            ...
            <table border="1" bordercolor="black" cellpadding="0" cellspacing="0"  class="table tableBf">
                ...
                <tr class="title">
                    <th :class="item.isSelected?'bfSelected':''" v-for="(item,i) in scoreObj.matchPlays?scoreObj.matchPlays[2].homeCell.cellSons:[]" :key='i'><b @click="unSelectedClickspf($event,item,'3',scoreObj.matchPlays[2].single)"></b><span>{{item.cellName}}</span> <span>{{item.cellOdds}}</span></th>
                </tr>
                <tr class="title">
                    <th :class="item.isSelected?'bfSelected':''" v-for="(item,i) in scoreObj.matchPlays?scoreObj.matchPlays[2].flatCell.cellSons:[]" :key='i'><b @click="unSelectedClickspf($event,item,'3',scoreObj.matchPlays[2].single)"></b><span>{{item.cellName}}</span> <span>{{item.cellOdds}}</span></th>
                </tr>
                <tr class="title">
                    <th :class="item.isSelected?'bfSelected':''" v-for="(item,i) in scoreObj.matchPlays?scoreObj.matchPlays[2].visitingCell.cellSons:[]" :key='i'><b @click="unSelectedClickspf($event,item,'3',scoreObj.matchPlays[2].single)"></b><span>{{item.cellName}}</span> <span>{{item.cellOdds}}</span></th>
                </tr>
            </table>
            <table border="1" bordercolor="black" cellpadding="0" cellspacing="0"  class="table tablezjq">
                <th rowspan="3" class="title0">总进球</th>
                <tr class="title">
                    <th :class="item.isSelected?'bfSelected':''" v-for="(item,i) in !scoreObj.flag?scoreObj.matchPlays[3].matchCells:[]" :key='i'><b @click="unSelectedClickspf($event,item,'4',scoreObj.matchPlays[3].single)"></b><span>{{item.cellName}}</span> <span>{{item.cellOdds}}</span></th>
                </tr>
            </table>
            <table border="1" bordercolor="black" cellpadding="0" cellspacing="0"  class="table tableBqc">
                <th rowspan="3" class="title0">半全场</th>
                <tr class="title">
                    <th :class="item.isSelected?'bfSelected':''" v-for="(item,i) in !scoreObj.flag?scoreObj.matchPlays[4].matchCells:[]" :key='i'><b @click="unSelectedClickspf($event,item,'5',scoreObj.matchPlays[4].single)"></b><span>{{item.cellName}}</span> <span>{{item.cellOdds}}</span></th>
                </tr>
            </table>
            </div>
