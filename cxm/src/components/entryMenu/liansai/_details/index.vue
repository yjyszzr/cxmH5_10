  ...
  <div class="detail">
    ...
    <div class="body">
      <div class="tops">
        ...
        <div class="top-right">
          <p class="name"><span v-if="datas.leagueType=='1'">联赛规则</span><span v-if="datas.leagueType=='0'">杯赛规则</span></p>
          <p class="rule" v-bind:class="openDetail?'openDetails':''" v-if="datas.leagueRule!=''">{{datas.leagueRule}}</p>
          <p class="detail-btn" @click='openDetail=!openDetail' v-show="datas.leagueRule&&datas.leagueRule.length>108"><span v-if="openDetail">收起</span><span v-if="!openDetail">详情</span></p>
        </div>
      </div>
      <ul class="tab-ul">
      ...
      <div class="container">
        ...
        <div class="containered score" v-if='navBodyIndex =="0"'>
          <ul class="score-nav-ul" v-if="leagueScore.matchType=='1'">
            <li class="score-nav-li" v-for="(item,index) in scoreNavArr" :key=index><span
              :class="item.cur?'scorecurtab':''" @click="scoreCick(item,index)">{{item.name}}</span><span
          ...
          <div class="score-main shoot">
            <div v-show="matchScoreDTOList.length>0" v-if="leagueScore.matchType=='1'">
              <div v-for="(item,index) in matchScoreDTOList" :key=index >
                <table border="0" cellspacing="0" cellpadding="0" class="hot-table" v-if="index==scoreMainIndex">
            ...
            <div v-show="matchScoreDTOList.length>0" v-if="leagueScore.matchType=='0'">
              <table border="0" cellspacing="0" cellpadding="0" class="hot-table" v-for="(item,index) in matchScoreDTOList" :key=index >
                <tr class="t-head">
                  <td colspan='9'>{{item.groupName}}</td>
                </tr>
                <tr class="hot-th">
          ...
          <div v-if="matchTurnGroupList.length>0">
            <div class="match-nav">
              ...
              <ul class="mach-select-ul">
                <li class="mach-select-li" v-if="item.cur" v-for='(item,index) in matchTurnGroupList' :key=index
                    @click="openMore()">
                  <span>{{item.turnGroupName}}</span>
                  <span ref="downImg"><img src="../../../../assets/img/freebuy_img/Collapse@3x.png" alt=""></span>
                </li>
              </ul>
            ...
            <div class="match-navbox" v-show="machItem">
              <ul class="math-ul">
                <li class="math-li" v-for='(item,index) in matchTurnGroupList' :key=index @click="openMore(item,index)">
                  <span>{{item.turnGroupName}}</span></li>
              </ul>
            </div>
            <div v-for="(item,index) in matchTurnGroupList" :key=index>
              <table v-show="item.cur" class="hot-table" v-for="(sunitem,index) in item.groupDTOList" :key=index>
                ...
                <tr class="hot-tr" v-for='(grandsunitem,index) in sunitem.futureMatchDTOList' :key=index>
        ...
        <div class="containered team" v-if='navBodyIndex =="3"'>
          <div v-show="leagueTeamInfoDTOList.length>0">
            <ul class="hot-ul">
              <li @click="goTeamBall(item)" class="hot-li" v-for="(item,index) in leagueTeamInfoDTOList" :key=index>
